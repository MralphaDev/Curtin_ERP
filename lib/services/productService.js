...
export async function createValveBody(data) {
  ...
  const [result] = await db.query(
    `INSERT INTO valve_body (model_number, category, manufacturer)
     VALUES (?, ?, ?)`,
    [data.model_number, data.category, data.manufacturer]
  );
...
export async function getValvesWithStock() {
  ...
  try {
    ...
    const [products] = await conn.query(`
      SELECT id, model_number, category, manufacturer
      FROM valve_body
